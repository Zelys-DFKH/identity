...
import { makeEvent, makeMergedPREvent, makeProfile, makeSyncPREvent } from "./helpers/make-event";

const ACCOUNT = "testuser";
const OWN_REPO = `${ACCOUNT}/my-repo`;
function expectExcludesOwnRepos(
	detect: (events: ReturnType<typeof makeEvent>[], account: string) => unknown[],
	eventType: string,
	count = 10,
): void {
	const events = Array.from({ length: count }, () =>
		makeEvent(eventType, OWN_REPO, "2024-01-01T00:00:00Z"),
	);
	expect(detect(events, ACCOUNT)).toHaveLength(0);
}
...
describe("detectReviewActivity", () => {
	...
	it("excludes reviews on own repos", () => {
		expectExcludesOwnRepos(detectReviewActivity, "PullRequestReviewEvent");
	});
});
...
describe("detectReviewCommentActivity", () => {
	...
	it("excludes review comments on own repos", () => {
		expectExcludesOwnRepos(detectReviewCommentActivity, "PullRequestReviewCommentEvent");
	});
});
...
describe("detectFollowerCount", () => {
	...
	it("flags high tier at 200+ followers", () => {
		const profile = makeProfile({ followers: 200 });
		const flags = detectFollowerCount(profile);
		expect(flags).toHaveLength(1);
...
describe("detectIdentityCompleteness", () => {
	...
	it("does not count a bio shorter than 20 characters", () => {
		const profile = makeProfile({ name: "Alice", bio: "Short", company: "Acme" });
		// name + company = 2, bio too short = not counted → below threshold
		expect(detectIdentityCompleteness(profile)).toHaveLength(0);
	});

	it("flags base tier with 3 valid fields", () => {
		const profile = makeProfile({ name: "Alice", company: "Acme Corp", location: "Berlin" });
	...
	it("flags high tier with all 5 fields including long bio", () => {
		const profile = makeProfile({ name: "Alice", company: "Acme Corp", location: "Berlin", blog: "https://example.com", bio: "I write software and love open source contributions." });
		const flags = detectIdentityCompleteness(profile);
		expect(flags).toHaveLength(1);
