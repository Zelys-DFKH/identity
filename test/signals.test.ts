...
const BASE = { createdAt: "2025-01-01T00:00:00Z", reposCount: 10, accountName: "user" };
...
describe("identify - Fork Surge Detection", () => {
	it("should flag multiple forks (5-7 in 24 hours)", () => {
		...
		const result = identify({ ...BASE, events });
	...
	it("should not flag forks spread over more than 24 hours", () => {
		...
		const result = identify({ ...BASE, events });
		expect(result.flags.some((f) => f.label.includes("fork") || f.label.includes("Fork"))).toBe(false);
	});

	it("should not show sustained fork rate on single-day spikes", () => {
		const events: GitHubEvent[] = Array.from({ length: 17 }, (_, i) => ({
			type: "ForkEvent",
		...
		const result = identify({ ...BASE, events });
		expect(result.flags.some((f) => f.label === "Fork spike detected")).toBe(true);
		expect(result.flags.some((f) => f.label === "Sustained fork rate")).toBe(false);
	});
	...
	it("should not show fork scatter pattern when spike is detected", () => {
		...
		const result = identify({ ...BASE, events });
		expect(result.flags.some((f) => f.label.includes("Severe fork surge") || f.label.includes("Extreme"))).toBe(true);
		expect(result.flags.some((f) => f.label === "Fork scatter pattern")).toBe(false);
	});
	...
	it("should show fork scatter pattern for slow, distributed targeting", () => {
		...
		const result = identify({ ...BASE, events });
		expect(result.flags.some((f) => f.label.includes("spike") || f.label.includes("Spike"))).toBe(false);
		expect(result.flags.some((f) => f.label === "Fork scatter pattern")).toBe(true);
	});
});
	...
	it("should flag concentrated repository creation (16+ repos in 24 hours)", () => {
		const events: GitHubEvent[] = [
			...Array.from({ length: 16 }, (_, i) => ({
				type: "CreateEvent",
				created_at: new Date(Date.UTC(2026, 2, 10, Math.floor(i / 2), 0, 0)).toISOString(),
				repo: { name: `repo${i}` } as any,
				payload: { ref_type: "repository" },
			} as any)),
			{ type: "PushEvent", created_at: new Date(Date.UTC(2026, 2, 10, 8, 0, 0)).toISOString(), repo: { name: "extra1" } as any } as any,
			{ type: "PushEvent", created_at: new Date(Date.UTC(2026, 2, 10, 9, 0, 0)).toISOString(), repo: { name: "extra2" } as any } as any,
		];

		const result = identify({ ...BASE, reposCount: 100, events });
		expect(result.flags.some((f) => f.label === "Concentrated repository creation")).toBe(true);
	});

	it("should ignore CreateEvent that are branch creations, not repos", () => {
		const events: GitHubEvent[] = Array.from({ length: 5 }, (_, i) => ({
			type: "CreateEvent",
			created_at: new Date(Date.UTC(2026, 2, 10, i, 0, 0)).toISOString(),
			repo: { name: `repo${i}` } as any,
			payload: { ref_type: "branch" },
		} as any));

		const result = identify({ ...BASE, events });
		expect(result.flags.some((f) => f.label.includes("Concentrated repository creation"))).toBe(false);
	});
});
	...
	it("should not flag 24/7 pattern if activity is spread over multiple days", () => {
		...
		for (let day = 0; day < 5; day++) {
			for (let hour = 8; hour < 20; hour++) {
				events.push({ type: "PushEvent", created_at: new Date(2026, 2, 10 + day, hour, 0, 0).toISOString(), repo: { name: "repo" } as any } as any);
			}
		}

		const result = identify({ ...BASE, reposCount: 5, events });
		expect(result.flags.some((f) => f.label === "24/7 activity pattern")).toBe(false);
	});
});
...
describe("identify - Narrow Activity Focus", () => {
	it("should flag narrow activity focus (few event types, no interactions)", () => {
		...
		const result = identify({ ...BASE, reposCount: 5, events });
		expect(result.flags.some((f) => f.label === "Narrow activity focus")).toBe(true);
	});
	...
	it("should not flag narrow focus if there are human interactions", () => {
		const events: GitHubEvent[] = [
			...
			{ type: "IssueCommentEvent", created_at: new Date(2026, 2, 10, 13, 0, 0).toISOString(), repo: { name: "repo" } as any } as any,
		];

		const result = identify({ ...BASE, reposCount: 5, events });
		expect(result.flags.some((f) => f.label === "Narrow activity focus")).toBe(false);
	});
});
...
describe("identify - Score Calculation", () => {
	...
	it("should cap score at 0 minimum", () => {
		const events: GitHubEvent[] = Array.from({ length: 50 }, (_, i) => ({
			type: "ForkEvent",
			created_at: new Date(2026, 2, 10, Math.floor(i / 2), i % 60, 0).toISOString(),
			repo: { name: `repo${i}` } as any,
		} as any));

		const result = identify({ createdAt: "2026-03-08T00:00:00Z", reposCount: 0, accountName: "bot", events });
		expect(result.score).toBeGreaterThanOrEqual(0);
	});
});
...
describe("identify - Classification", () => {
	...
	it("should classify as mixed when score is between 50-69", () => {
		const events: GitHubEvent[] = [
			...
			{ type: "PushEvent", created_at: new Date(2026, 2, 10, 10, 0, 0).toISOString(), repo: { name: "org/shared-repo" } as any } as any,
		];
	...
	it("should classify as automation when score < 50", () => {
		const events: GitHubEvent[] = Array.from({ length: 35 }, (_, i) => ({
			type: "ForkEvent",
...
describe("identify - Profile Information", () => {
	it("should include correct account age in profile", () => {
		const result = identify({ createdAt: "2025-12-01T00:00:00Z", reposCount: 5, accountName: "user", events: [] });
	...
	it("should include repos count in profile", () => {
		const result = identify({ ...BASE, reposCount: 42, events: [] });
...
describe("identify - Issue Comment Spam Detection", () => {
	it("should flag extreme issue comment spam (15+ repos in 2 minutes)", () => {
		const events: GitHubEvent[] = [
			...
			{ type: "PushEvent", created_at: new Date(2026, 2, 10, 12, 2, 0).toISOString(), repo: { name: "owner/main" } as any } as any,
		];

		const result = identify({ ...BASE, reposCount: 30, events });
	...
	it("should flag issue comment spray at the ISSUE_COMMENT_SPRAY_HIGH boundary (6 repos)", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
	...
	it("should not flag issue comment spray just below the ISSUE_COMMENT_SPRAY_HIGH boundary (5 repos)", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		expect(result.flags.some((f) => f.label === "High comment frequency across repos" || f.label === "Issue comment spam")).toBe(false);
	});
	...
	it("should not flag issue comments spread over longer time periods", () => {
		const events: GitHubEvent[] = [
			...
			{ type: "PushEvent", created_at: new Date(2026, 2, 10, 13, 0, 0).toISOString(), repo: { name: "owner/main" } as any } as any,
		];

		const result = identify({ ...BASE, reposCount: 30, events });
		expect(result.flags.some((f) => f.label === "Issue comment spam" || f.label === "High comment frequency across repos")).toBe(false);
	});
	...
	it("should properly calculate comments per minute in flag detail", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		const issueCommentFlag = result.flags.find((f) => f.label === "Issue comment spam" || f.label === "High comment frequency across repos");
		expect(issueCommentFlag).toBeDefined();
		if (issueCommentFlag) {
	...
	it("should count distinct repos, not total comments, when determining spray severity", () => {
		...
		const events: GitHubEvent[] = Array.from({ length: 15 }, (_, i) => ({
			type: "IssueCommentEvent",
			created_at: new Date(2026, 2, 10, 12, 0, i * 8).toISOString(),
			repo: { name: `owner/repo${repos[i % repos.length]}` } as any,
		} as any));
...
describe("identify - PR Comment Spam Detection", () => {
	...
	it("should flag high PR comment frequency (8-11 PRs in short timeframe)", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		expect(result.flags.some((f) => f.label === "High PR comment frequency")).toBe(true);
	});
	...
	it("should not flag PR comments spread over longer time periods", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		expect(result.flags.some((f) => f.label === "PR comment spam" || f.label === "High PR comment frequency")).toBe(false);
	});
	...
	it("should include correct comment count and repo count in flag detail", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		const prCommentFlag = result.flags.find((f) => f.label === "High PR comment frequency");
	...
	it("should properly calculate comments per minute in flag detail", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		const prCommentFlag = result.flags.find((f) => f.label === "PR comment spam" || f.label === "High PR comment frequency");
		expect(prCommentFlag).toBeDefined();
		if (prCommentFlag) {
	...
	it("should assign correct points for high PR comment frequency", () => {
		...
		const result = identify({ ...BASE, reposCount: 20, events });
		const prFreqFlag = result.flags.find((f) => f.label === "High PR comment frequency");
...
describe("identify - Extreme PR Spam Detection (Time-Based)", () => {
	...
	it("should not flag legitimate long-term activity (500 PRs over 6 months)", () => {
		const events: GitHubEvent[] = Array.from({ length: 500 }, (_, i) => {
			const repoIndex = i % 30;
			const daysAgo = Math.floor(i / 2.7);
			return {
				type: "PullRequestEvent",
				payload: { action: "opened" },
		...
		const result = identify({ ...BASE, reposCount: 35, events });
		const extremeDailyFlag = result.flags.find((f) => f.label === "Extreme PR spam (daily)");
