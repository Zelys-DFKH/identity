...
export function detectImpossibleThroughput(
	events: GitHubEvent[],
): IdentifyFlag[] {
	const writeEvents = events
		.filter((e) => e.type && WRITE_TYPES.has(e.type) && e.created_at)
		.map((e) => ({ ts: new Date(e.created_at as string).getTime() }))
	...
	for (let i = 0; i < writeEvents.length; i++) {
		if (j < i) j = i;
		while (
			j < writeEvents.length &&
			writeEvents[j].ts - writeEvents[i].ts < CONFIG.THROUGHPUT_WINDOW_MS
		)
			j++;
		const count = j - i;
